...
class _VideoMenuBottomSheetState extends State<VideoMenuBottomSheet>
  ...
  Widget _buildMenuOptions(BuildContext context, ThemeService themeService) {
    ...
    // 其他来源显示菜单
    // 根据上映状态决定显示的选项
    final releaseStatus = widget.videoInfo.releaseStatus;
    final isUpcoming = releaseStatus != null && releaseStatus.contains('天后上映');
    final isReleased = releaseStatus != null &&
        (releaseStatus == '今日上映' || releaseStatus.startsWith('已上映'));
    // 是否来自即将上映列表
    final isFromUpcoming = isUpcoming || isReleased;

    List<Widget> menuItems = [];

    // 播放记录来源始终显示播放选项（用户已观看过，说明可播放）
    // 或者已上映/今日上映的视频也显示播放选项
    if (widget.from == 'playrecord' || isReleased) {
      menuItems.addAll([
        _buildMenuItem(
          context,
          themeService,
